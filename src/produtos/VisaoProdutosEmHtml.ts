...
export class VisaoProdutosEmHtml implements VisaoProdutos {
    ...
    listar(produtosPaginados: ProdutosPaginados): void {
        const ancoraCarrinho = document.getElementById('carrinho') as HTMLAnchorElement;

        ancoraCarrinho.classList.remove('invisible');

        this.listarProdutos(produtosPaginados.produtos);

        this.listarPaginas(produtosPaginados.totalPaginas, produtosPaginados.paginaAtual);
    ...
    detalhar(produto: ProdutoParaDetalhar): void {
        ...
        const lancamento = document.getElementById('lancamento') as HTMLHeadingElement;
        const descricao = document.getElementById('descricao') as HTMLParagraphElement;
        const precoSemDesconto = document.getElementById('preco-sem-desconto') as HTMLSpanElement;
        ...
        const botaoEsgotado = document.getElementById('esgotado') as HTMLButtonElement;
        ...
        if (produto.precoPromocional) {
            ...
            precoSemDesconto.parentElement!.classList.add('text-decoration-line-through');
        ...
        if (produto.estoque > 0) {
            quantidade.min = '1';
            quantidade.max = produto.estoque >= 10 ? '10' : produto.estoque.toString();

            botaoAdicionar.addEventListener('click', (event) => {
                event.preventDefault();

                this.controladoraProdutos.adicionarAoCarrinho(produto.id, quantidade.value);
    ...
    private listarProdutos(produtos: ProdutoParaListar[]): void {
        ...
        for (const produto of produtos) {
            ...
            const precoSemDesconto = col.querySelector('.preco-sem-desconto') as HTMLSpanElement;
    ...
    private listarPaginas(totalPaginas: number, paginaAtual: number): void {
        const paginaTemplate = document.getElementById('pagina') as HTMLTemplateElement;

        const fragmentoPaginas = document.createDocumentFragment();

        for (let i = 1; i <= totalPaginas; i++) {
            const pageItem = paginaTemplate.content.cloneNode(true).firstChild as HTMLLIElement;
    ...
    private configurarPaginacao(paginaAtual: number, temAnt: boolean, temProx: boolean): void {
        ...
        if (temProx) {
            ...
            const pageLinkProx = pageItemProx?.querySelector('a') as HTMLAnchorElement;
